...
​	可差分的信息：什么是可差分的信息呢？比如累加和数组、累乘积数组等，知道了总体，知道其中一部分，可以直接求出剩下的部分。
...
​	怎样维持一个树状数组呢？这个数组和原始数组等长，且**下标都从1开始**。树状数组维护的信息区间，对应起来颇为特殊，也是精妙的地方。树状数组中下标对应维护的区间如下：1 -> 1，2 -> 1、2，3 -> 3，4 -> 1、2、3、4，5 -> 5，6 -> 5、6，7 -> 7，8 -> 1、2、3、4、5、6、7、8，等等。这个对应关系如何图解呢？将原始数组分组，组内元素个数从1开始，分完当前组长度之后回头再次分组，下次分组长度为上次乘以2，最后，原始数组中下标 i 位置处于某个组的最右侧，那么树状数组中的 i 位置就负责这个组（区间）。

​	那么如何求出树状数组中的下标 i 负责哪段区间呢？该区间的右边界一定是i本身，左边界就是i将最右边的1变成0再加1。

​	范围查询query方法应该如何实现呢？我们只需要求下标1到 i 范围的累加和即可。先将 i 位置在树状数组中的值拿出来，看看 i 位置负责原始数组的哪一个组，将该组的和加上，然后去掉 i 二进制下最右侧的1，再去找分组累加和，直到 i 变为0。这个过程一定是成倍增长的，也就是查的分组的长度每次乘以2，所以整体时间复杂度为O(logn)。

​	对于单点增加方法add，树状数组中不止一个位置负责 i 下标，所以要不断加上 i 最右侧的1，这些下标就是负责的区间有 i 的位置。
...
​	此处利用原数组的差分数组来构建树状数组，这样范围增加可以变成两条单点增加，单点查询也就变成了[1, i]求和。代码处理差分数组，和上面的没有区别。
...
![image-20250215091835473](typora-user-images/image-20250215091835473.png)
...
![image-20250215153226938](typora-user-images/image-20250215153226938.png)

![image-20250215153238394](typora-user-images/image-20250215153238394.png)
